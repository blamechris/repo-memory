...
This is a headless MCP server with no user interface, so Pathfinder should trace MCP tool-call flows (stdio transport → tool handler in `src/tools/` → cache/indexer/persistence layers) rather than user-facing request/response flows.
...
<!-- skill-templates: recon 7f5fa28 2026-06-08 -->
