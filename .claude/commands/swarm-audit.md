...
| Cache Correctness | "Cache" | Stale-data risks, hash/invalidation correctness, never-return-stale invariant | Target touches the cache engine, file hashing (SHA-256), or invalidation logic |
| MCP Protocol | "Protocol" | MCP tool contracts, stdio transport, schema/param shape, token-cost ROI of tools | Target adds or changes MCP tools, server handlers, or stdio I/O |
...
For cache/indexer/persistence targets, Cache and Guardian must weight the "cache correctness over performance — never return stale data" invariant and deterministic SHA-256 hashing: any path that can return stale or non-deterministic data is at most 2/5. For MCP tool/server targets, Protocol and Minimalist should weight the per-tool token cost (~100 tokens/turn) and ROI of (frequency × savings) − (turns × overhead) — prefer enhancing existing tools with optional params over new tools. All agents should flag any `console.log` in production code (structured logging via MCP only) and any non-ESM constructs.
...
<!-- skill-templates: swarm-audit c8b8477 2026-06-08 -->
