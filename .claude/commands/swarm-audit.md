...
| Cache Auditor | "Cache Auditor" | File hash correctness, cache invalidation, staleness detection, memory efficiency | Target involves caching, file hashing, or cache invalidation logic |
| MCP Compliance | "MCP Compliance" | MCP protocol correctness, tool/resource/prompt definitions, message format | Target involves MCP server implementation or protocol surface |
...
For repo-memory audits, apply these domain-specific standards:

- **Cache Auditor** should weight file hash correctness, cache invalidation patterns, and staleness detection heavily. A design that risks returning stale data is a blocker.
- **MCP Compliance** should verify tool definitions (get_file_summary, get_changed_files, get_project_map), resource schemas, and message format compliance against the MCP spec.
- **Guardian** should focus on race conditions in file watching/polling, concurrent cache updates, and data integrity across session boundaries.
- **Builder** should estimate effort in terms of TypeScript strict mode compliance, async/await patterns, and integration with file system APIs and Git.
...
/swarm-audit "the cache invalidation strategy in src/cache.ts" 6
/swarm-audit docs/mcp-protocol-surface.md 5
/swarm-audit "file hash correctness and staleness detection" 4
```
