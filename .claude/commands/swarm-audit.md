...
| MCP Compliance | "MCP Compliance" | MCP protocol correctness, tool/resource/prompt definitions, client compatibility | Target involves MCP server implementation or protocol surface |
...
For repo-memory audits, prioritize:
- **Cache Auditor**: File hash correctness and determinism. Cache invalidation on file changes. Staleness detection accuracy. Memory efficiency with large repos (10k+ files).
- **MCP Compliance**: Tool definitions (get_file_summary, get_changed_files, get_project_map). Resource and prompt correctness. Client compatibility with Claude and other MCP hosts.
- **Guardian**: Race conditions in file watching / polling. Data integrity during concurrent cache updates. Recovery from corrupted cache state.
- **Builder**: TypeScript strict mode compliance. Async/await patterns and error handling. Persistence layer (JSON/SQLite) reliability.
...
/swarm-audit "the file hash and cache invalidation logic in src/cache.ts" 6
/swarm-audit docs/architecture.md
/swarm-audit "MCP server tool definitions and protocol compliance" 5
```
