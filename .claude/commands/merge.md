...
- `$ARGUMENTS` - PR numbers, `all`, or flags:
  ...
  - `--no-build` — skip the post-merge build verification
  - `--build-only` — run the post-merge build on current main without merging
...
Pure documentation/skill file changes (`.md` files under `docs/` or `.claude/`) with zero code changes may skip review.
...
This repo has no auto-version CI; version bumps are manual and land via their own PR
(see git log: `chore: bump version to 0.4.0`). The version source of truth is the root
`package.json` (npm package `@blamechris/repo-memory`).
...
gh pr create --title "chore: bump version to ${NEXT}" --body "Patch version bump."
```

Merge after CI passes (version-only change, review gate exception). Publishing to npm
(`npm publish`) is a separate manual step the user runs after the bump lands.
...
- Merged PRs only touch docs (`docs/`, `*.md`) or `.claude/` skill files — no build needed
...
#### Step 3b: Verify the build

The package is published from compiled `dist/` (entry `src/server.ts` → `dist/server.js`),
so confirm main still builds and typechecks cleanly after the merge:
...
If any step fails, report the error and stop — do not proceed to a version bump or publish
on a broken main.
...
**Build:** typecheck + lint + test + build passed on main
```
...
7. **Never publish to npm from this command** — `npm publish` requires OTP and is a deliberate manual step run by the user after a version bump PR lands
<!-- skill-templates: merge d34b05d 2026-06-08 -->
