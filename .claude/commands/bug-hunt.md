...
Guardian must treat **stale-cache correctness** as the top-priority bug class for this repo: the project's hard rule is "cache correctness over cache performance — never return stale data." Specifically check that any change to hashing, invalidation, or SQLite store/load cannot return data for a file whose content changed, and that SHA-256 file hashing stays deterministic (same bytes → same hash). A path that serves a cached summary after the underlying file mutated is a critical finding, not a perf note.
...
Match this repo's label conventions. The base label is `bug`. Where the target maps cleanly to a subsystem, add the matching `epic:*` label (`epic:cache-engine`, `epic:mcp-server`, `epic:indexer`, `epic:task-memory`, `epic:graph`, `epic:persistence`, `epic:telemetry`, `epic:infra`, `epic:dx`). Optionally add a `complexity:{low,medium,high}` and/or `testing:{low,medium,high}` label when the fix scope is clear. Confirm each label exists via `gh label list` and skip any that do not.
...
When the target touches `src/cache/`, `src/indexer/`, or `src/persistence/`, Guardian MUST explicitly sweep for the stale-data class: any code path that can return a cached file summary, dependency edge, or task record after the underlying file content changed (SHA-256 hash mismatch not detected, invalidation skipped, or a save that defaults-then-writes over loaded state). This class is mandatory to report when in scope, at `critical` severity.
