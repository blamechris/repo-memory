...
- `docs/planning/` holds design docs (e.g., `*-design.md`) that may contain unbuilt plans
...
```bash
# Source file patterns to scan: *.ts under src/
...
| P1 | High | `complexity:low`/`complexity:medium`, `good first issue`, `help wanted`, `from-review` issues, milestone-assigned issues, stale PRs |
