...
| Category | Detection |
|----------|-----------|
| Blocked | Has label matching: `blocked`, `wontfix`, `needs-design-spike` |
| Assigned | Has assignees (someone is already working on it) |
| Ready | `complexity:low` or `complexity:medium` with acceptance criteria in the body; milestone `V1` takes priority over `V2`/`V3` |
...
# Check repo-specific planning files
for f in docs/planning/architecture.md; do
  test -f "$f" && echo "Found: $f"
done
```
...
grep -r "TODO\|FIXME\|HACK\|XXX\|WORKAROUND" src/**/*.ts --exclude-dir=node_modules --exclude-dir=.godot --exclude-dir=build --exclude-dir=dist 2>/dev/null || true
```
...
| P1 | High | `complexity:low` or `complexity:medium` with acceptance criteria, `from-review` issues, `V1` milestone issues, stale PRs |
| P2 | Normal | `complexity:high` issues with acceptance criteria, `from-audit` issues, roadmap items, `V2`/`V3` milestone issues |
...
**Blocked / Needs input:**
- #30 — Choose auth provider (`needs-design-spike`) — requires decision
...
| FIXME | 3 | src/auth/token.ts (2), src/db/migrate.ts (1) |
