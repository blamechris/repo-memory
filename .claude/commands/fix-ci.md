...
```bash
MAX_WAIT=600  # seconds (GitHub-hosted ubuntu-latest, single Node 20.x matrix job)
INTERVAL=30
ELAPSED=0
...
Project-specific patterns (repo-memory — headless TypeScript/Node 20+ ESM MCP server; the CI `ci` job runs typecheck → lint → test → build in sequence):
- Lint step fails while typecheck passed (e.g. unused vars / `@typescript-eslint/no-unused-vars`) → FIX. **Known footgun:** `npm run typecheck` can pass while `npm run lint` FAILS — these are separate gates, so always run `npm run lint` locally before pushing the fix. Don't trust a "typecheck passes" claim.
- `npm test` / `vitest` failures → reproduce locally with `npm test` (and `npm run test:integration` for integration); FIX if a clear code/test issue, ESCALATE if behavior is ambiguous.
- `npm run build` (tsc) fails after typecheck/lint passed → FIX (usually emit/module-resolution; ESM + NodeNext, check extensions on relative imports).
- `better-sqlite3` native binding / `NODE_MODULE_VERSION` / ABI mismatch / `Could not locate the bindings file` (esp. on a new Node major) → the prebuilt is pinned to a Node ABI. FIX by reinstalling to fetch the matching prebuilt (`npm ci`, do NOT source-rebuild); if no prebuilt exists for that Node major, ESCALATE (pin the Node version or bump better-sqlite3).
...
# Fallback if gh run rerun is unavailable (e.g. run too old/expired): push an
# empty commit to re-trigger the full suite, or close/reopen the PR.
#   git commit --allow-empty -m "chore(infra): re-trigger CI" && git push
```
...
<!-- skill-templates: fix-ci 3768ea6 2026-06-08 -->
