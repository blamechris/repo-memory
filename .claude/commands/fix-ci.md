...
repo-memory-specific patterns:
- `Cannot find module` / `ERR_MODULE_NOT_FOUND` → Check `npm install` ran; verify `package.json` and lockfile consistency
- `TypeScript error TS` → `npm run typecheck` failed; check type annotations and strict mode compliance
- `Build failed` / `tsc` error → `npm run build` failed; likely TypeScript or bundler issue
- `Test timeout` / `Jest timeout` → Cache correctness test or file system operation hanging; check for deadlocks or missing cleanup
- `Hash mismatch` / `cache invalidation` → File hash computation or cache invalidation logic broken; verify deterministic hashing
- `MCP protocol` / `tool not found` → Tool handler registration or response format issue; check MCP server compliance
