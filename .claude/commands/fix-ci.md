...
- `npm ERR!` with `ERESOLVE` or dependency conflict → Check `package-lock.json` and `npm outdated`; may need `npm install --legacy-peer-deps` or dependency upgrade
- `TypeScript error TS` → Run `npm run typecheck` locally; fix type errors before pushing
- `Test timeout` or `jest timeout` → Increase test timeout or optimize slow tests; check for hanging async operations
- `Cache invalidation mismatch` → Verify file hash computation matches expected; check for non-deterministic hashing or stale cache entries
- `MCP protocol error` → Validate tool/resource/prompt definitions against MCP spec; check JSON schema compliance
