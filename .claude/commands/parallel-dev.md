...
# Test file conventions: tests/unit/*.test.ts (unit), tests/integration/*.test.ts (integration), fixtures in tests/fixtures/
# Test runner: npm test (unit, vitest); npm run test:integration for integration tests
...
# Verification (run all four — typecheck alone can pass while lint fails on unused vars):
