...
- `$ARGUMENTS` - Issue source and options. Same as `/autonomous-dev-flow` plus marathon-specific options:
  ...
  - `label:complexity:low max:10 sort:created-asc` (with options)
...
| # | Issue | Labels | Action |
|---|-------|--------|--------|
| 1 | #12 — Add retry logic | complexity:low | Implement |
| 2 | #15 — Add leaderboard | complexity:high | Decompose → sub-issues |
| 3 | #18 — Auth integration tests | testing:medium | Implement |
