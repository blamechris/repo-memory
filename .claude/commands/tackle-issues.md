...
- **Shed context at each wave boundary — mode-aware.** When a wave completes (after Phase 2 replenishment and the Phase 4 convergence check), write/refresh the handoff note (`$CLAUDE_BRIEF_DIR/../handoffs/repo-memory-<date>-handoff.md`) and the durable queue (`scratchpad/autonomous-queue.json`), update the STATE header, then:
  ...
  - **Unattended with a configured re-launcher** (no wave re-launcher is configured for repo-memory — no scheduled trigger, cron/launchd job, or `/loop` wrapper drives this marathon; treat this case as absent and use the next bullet) — end the session; the re-launcher starts the next wave from the same seeds.
...
- **Per-wave cost circuit breaker.** At each wave boundary, check session cost. Over budget → write the handoff and **stop and notify**; do not start the next wave. This breaker is the sole sanctioned exception to Critical Rule 4's "everything after is fully autonomous".
...
# Branch prefix for NEW session branches this skill creates: auto/
BRANCH_PREFIX="auto/"

# Regex of every prefix a session branch might carry (this repo also uses feat/fix/refactor/test/docs/chore for manual work)
BRANCH_PREFIX_RE="^(auto|feat|fix|refactor|test|docs|chore)/"
```
