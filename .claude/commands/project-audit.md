...
**repo-memory note:** This is a headless MCP server (no UI, no human end user). Adjust the core personas accordingly. Strategist evaluates MCP tool completeness through the agentic-coding lens — does each tool earn its ~100 tokens/turn system-prompt cost (ROI = frequency × savings − turns × overhead), are the right tools exposed, are edge states (empty cache, stale entries, deleted/renamed files) handled — rather than human-facing feature polish. Sentinel treats the target project's filesystem and the SQLite cache as the attack surface (path traversal during scanning, untrusted file contents, SQL handling, secrets accidentally summarized into the cache) rather than web vulns (no XSS/CSRF here).
...
**repo-memory conventions:** Title issues as `type(scope): description` — types `feat|fix|refactor|test|docs|chore`, scopes `server|cache|indexer|memory|graph|telemetry|infra` (e.g. `fix(cache): invalidate on file rename`). Keep the `from-audit` label. Never add any AI/Claude attribution to issue titles or bodies — the user is the sole author.
...
**repo-memory grading criteria:** Treat **cache correctness as a gating concern** — any path that can return stale or wrong cached data is a Critical finding and should cap the relevant area at 2/5 regardless of other strengths ("cache correctness over cache performance"). Curator weights hash determinism (SHA-256) and invalidation completeness above raw throughput. Envoy weights MCP schema correctness, stdio transport hygiene (no `console.log` in production code reaching stdout), and per-tool token ROI. Sentinel weights filesystem/SQLite attack surface (path traversal, untrusted file contents, secrets leaking into the cache) over web vulns. Inspector should weight that both `tests/unit` and `tests/integration` cover invalidation and edge cases (deleted/renamed files, empty cache).
