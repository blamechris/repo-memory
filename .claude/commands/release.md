...
`npm version <patch|minor|major>` — this updates `package.json` and, by default, also creates a git commit **and** tag. Because the bump already tags, step 7 must not double-tag.
...
- Create an annotated tag for the new version (skip if step 3's tool already tagged — `npm version` already created the commit and tag).
- Push the commit and the tag to the remote `origin main`. Direct pushes to `main` are forbidden (branch protection) — the version-bump commit must land via PR, not a direct push.
