...
For this repo (a headless MCP server), durable knowledge tends to be: MCP tool/protocol quirks (stdio transport, tool schema constraints), SQLite/better-sqlite3 behavior, cache correctness/invalidation gotchas, indexer/summarizer edge cases, and non-obvious build/test (vitest) constraints. General TypeScript or Node knowledge does not qualify.
...
When routing to CLAUDE.md, target the matching existing section rather than appending blindly to the end. This repo's CLAUDE.md sections are: `## Architecture`, `## Key Commands`, `## Code Conventions`, `## Git Workflow` (with `### Zero Attribution Policy`), `## Testing`, and `## Project Structure`. The `src/` layout (`tools/`, `cache/`, `indexer/`, `persistence/`, `graph/`, `memory/`, `telemetry/`) mirrors the commit scopes -- use the matching scope name when an insight is component-specific.
...
- **`.claude/rules/`:** `mkdir -p .claude/rules` then create the file using kebab-case names that match the commit scope where sensible (e.g., `cache-invalidation.md`, `indexer.md`, `persistence.md`). Include `paths:` frontmatter when scoped to directories (e.g., `src/cache/**`, `src/indexer/**`, `src/persistence/**`, `tests/**`).
...
<!-- skill-templates: learn df2de5e 2026-06-08 -->
