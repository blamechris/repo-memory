...
1. File hash invalidation must account for symlinks -- two symlinks to the same file should not share cache entries
   Evidence: VERIFIED -- tested with symlink fixtures, confirmed cache collision
   Before/After: Hash file path only --> Hash canonical path after symlink resolution

1. File hash symlink handling --> .claude/rules/cache-invalidation.md -- awaiting approval

+ - Resolve symlinks to canonical paths before hashing. Two symlinks to the same file must have separate cache entries to prevent stale data.
...
1. MCP tool response size exceeds 100KB causes client timeout -- chunk large summaries into multiple tool calls
   Evidence: VERIFIED -- profiler showed timeout at 120KB response
   Before/After: Return full summary in one tool call --> Split large summaries across multiple calls

2. TypeScript strict mode catches import cycle detection bugs early
   Evidence: OBSERVED -- caught circular dependency during typecheck

Persisted 1 of 2 insights.
1. MCP response chunking --> .claude/rules/mcp-protocol.md -- awaiting approval
2. TypeScript strict mode --> skipped (already in CLAUDE.md ## Architecture)

+ - MCP tool responses over 100KB cause client timeout. Chunk large file summaries into multiple tool calls (e.g., by section or file group).
...
```
User: /learn File hash must use content hash, not mtime -- mtime changes on git checkout without content change

1. File hash content-based approach --> .claude/rules/cache-invalidation.md -- awaiting approval

+ - Use content-based hashing (SHA256 of file contents), not mtime. Git operations change mtime without changing content, causing false cache invalidations.
...
1. Cache invalidation should check file size before hashing -- size mismatch is faster than full hash
   Evidence: VERIFIED -- benchmark showed 10x speedup on large repos
   Before/After: Always hash full content --> Check size first, hash only if size matches

Persisted 0 of 1 insights (conflict found).
1. Cache size-check optimization --> CONFLICTS with .claude/rules/cache-invalidation.md line 5
   Existing: "Always use content hash for correctness; size checks are unreliable"
   Found:    "Check size first for performance; hash only if size matches"
   Action needed: keep existing / replace / keep both
```
...
1. MCP server must validate tool argument types before processing -- missing validation caused silent failures
   Evidence: VERIFIED -- added type guards, caught 3 edge cases in integration tests
   Before/After: Trust caller to pass correct types --> Validate all tool arguments at handler entry

2. Currently debugging cache invalidation on large monorepos (50K+ files)

1. MCP argument validation --> .claude/rules/mcp-protocol.md -- awaiting approval
2. Current WIP context --> CLAUDE.local.md -- applied

+ - Validate all MCP tool argument types at handler entry. Missing validation causes silent failures when clients pass unexpected types.
