...
1. File hash invalidation must account for symlinks -- two symlinks to the same file should share one cache entry
   Evidence: VERIFIED -- tested with symlink fixtures, hash collision detection works
   Before/After: Hash each file path independently --> Resolve symlinks before hashing to deduplicate cache entries

1. File hash symlink deduplication --> .claude/rules/cache-invalidation.md -- awaiting approval

+ - Resolve symlinks before computing file hashes to ensure cache deduplication. Two symlinks to the same file must map to a single cache entry, not duplicate summaries.
...
1. MCP tool handlers must validate input schema before processing -- missing validation causes silent failures in client parsing
   Evidence: VERIFIED -- debugged client-side parse error traced to unvalidated tool output
   Before/After: Trust input types from TypeScript --> Always validate tool input against declared schema

2. Cache invalidation on git checkout requires polling, not just file system events
   Evidence: OBSERVED -- file system watcher missed changes during rapid branch switches

Persisted 1 of 2 insights.
1. MCP tool input validation --> .claude/rules/mcp-protocol.md -- awaiting approval
2. Git checkout cache invalidation --> skipped (already in .claude/rules/cache-invalidation.md)

+ - MCP tool handlers must validate input against declared schema before processing. TypeScript types do not guarantee runtime safety; validation prevents silent client-side parse failures.
...
```
User: /learn File hash must be deterministic across Node.js versions -- use SHA256 with explicit encoding, not crypto.createHash default

1. File hash determinism --> .claude/rules/cache-invalidation.md -- awaiting approval

+ - File hashes must be deterministic across Node.js versions. Use explicit SHA256 with UTF-8 encoding; do not rely on crypto.createHash defaults which may vary.
...
1. MCP resources should expose file summaries, not raw file contents -- reduces token waste and prevents stale data
   Evidence: VERIFIED -- benchmarked resource vs. tool approach, resource-based summaries 40% cheaper
   Before/After: Expose raw file contents via resources --> Expose pre-computed summaries via resources, fetch raw content via tools only

Persisted 0 of 1 insights (conflict found).
1. MCP resource design --> CONFLICTS with .claude/rules/mcp-protocol.md line 8
   Existing: "Resources expose raw file contents for client-side filtering"
   Found:    "Resources should expose pre-computed summaries to reduce token waste"
   Action needed: keep existing / replace / keep both
```
