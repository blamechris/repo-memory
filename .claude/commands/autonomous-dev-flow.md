...
- `$ARGUMENTS` - Issue source and options. Examples:
  - `label:complexity:low` (all open issues with this label)
  - `milestone:"V1"` (all open issues in milestone)
  - `#12 #15 #18` or `12 15 18` (specific issues by number)
  - `label:complexity:low max:5 sort:created-asc` (with options)
...
| # | Issue | Labels | Action |
|---|-------|--------|--------|
| 1 | #12 — Add retry logic to API client | complexity:low, testing:medium | Implement |
| 2 | #15 — Add leaderboard system | complexity:high, testing:high | Decompose → sub-issues |
| — | #16 — Refactor auth module | complexity:medium | Assigned to @user (skipped) |
| 3 | #18 — Add integration tests for auth flow | complexity:low, testing:medium | Implement |
...
- Issues labeled `blocked`, `wontfix`, or `needs-design-spike`
...
Smoke test is not applicable for this repo (Node.js/MCP server with no UI).
