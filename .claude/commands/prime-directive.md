...
Self-merge is **not** authorized for repo-memory. This repo does not grant unattended merge authority — PRs accumulate for human review (or `/batch-merge` at explicit user request), and the merge gate below stops at "ready to merge": run every gate, record the clean verdict, but do not execute the merge.
...
2. **Understand** — read the issue + linked threads. This repo ships the `repo-memory` MCP itself and it is available during dev sessions — prefer `get_file_summary` / `get_project_map` / `get_related_files` / `search_by_purpose` over raw Read/grep for code you won't edit. Re-verify any stored audit/plan claim against current main — audits go stale as main moves.
...
4. **Implement (TDD)** — branch `auto/<number>-<slug>` (this repo's automated-work convention), then RED → GREEN → REFACTOR. Match house style: "TypeScript strict mode, ES2022/NodeNext ESM, single quotes, trailing commas, 100-char lines (Prettier), no `console.log` in production (structured logging via MCP only)". Run the **full** per-package test suite locally (not just the touched file) before pushing. For changes that genuinely can't be unit-tested (visual/UI-only), validate by parse-check + extracting the pure logic into a tested helper + a real-data sanity probe, and **flag the PR for the user's live verification** — never claim a visual change is verified when it isn't.
...
6. **Full review (MANDATORY)** — run `/full-review`. A sub-agent review is mandatory on **every** PR (read-only: `gh pr diff` / `git show <ref>:<path>`; a non-worktree review agent must **never** `git checkout`). This repo's branch protection does not require a third-party (Copilot) review — treat any Copilot review as best-effort informational input only; never stall the merge gate waiting for it. Triage every thread.
...
1. **Shed context at every wave boundary — how depends on the run mode.** When a wave completes, write/refresh the handoff note, update the STATE header, then apply the case that matches this run:
   ...
   - **Unattended, re-launcher configured** — **end the session**; the re-launcher starts the next wave from the same three seeds. (No wave re-launcher is configured for repo-memory — no scheduled trigger, cron/launchd job, or `/loop` wrapper drives this marathon; treat this case as absent and use the next bullet.)
...
Run a **SOLID + DRY** whole-project audit (`/project-audit`) and file / act on its findings, then write the end-of-run report (below).
...
- **better-sqlite3 ABI** — on Node 26 it is prebuilt-only (cannot compile from source); fix an ABI mismatch (`NODE_MODULE_VERSION` error) with `npm install better-sqlite3`, **never** `npm rebuild` (deletes the working binary and forces a failing source compile).
- **web-tree-sitter pin** — pinned to `^0.25.10`; 0.26.x rejects the `tree-sitter-wasms` grammar binaries (Emscripten dylink mismatch at `Language.load`). Re-test grammar loading before any bump.
- **Lint ≠ typecheck** — `npm run typecheck` passing does not mean `npm run lint` passes (unused vars slip through); run both independently, plus `npm test` and `npm run build`, before merging.
- **Cache correctness over performance** — never return stale data. Any change to hashing, invalidation, or SQLite store/load must not serve a cached summary after the underlying file content changed; SHA-256 hashing must stay deterministic (same bytes → same hash).
- **ESM only, no `console.log`** — `"type": "module"` (NodeNext); structured logging via MCP only — a stray `console.log` corrupts the stdio transport.
...
- **Wave handoff note:** `$CLAUDE_BRIEF_DIR/../handoffs/repo-memory-<date>-handoff.md`. The session-boundary seed: queue position, open blockers, the awaiting-user list, the last verified merge. It is **disposable by design** — everything in it is re-derivable from GitHub — a seed for the next wave's session, never a second source of truth.
