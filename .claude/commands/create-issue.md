...
- `$ARGUMENTS` - Issue title (required). Optionally followed by flags:
  ...
  - `--milestone V1|V2|V3|Hardening` — Set milestone
  - `--epic NAME` — Epic label (e.g., mcp-server, cache-engine, indexer, task-memory, telemetry, infra)
...
# Add complexity label (required in repo-memory)
if [ -n "$COMPLEXITY" ]; then
  LABELS="$LABELS,complexity:$COMPLEXITY"
fi

# Add testing label (required in repo-memory)
if [ -n "$TESTING" ]; then
...
|-------|-------|--------|--------|
| #${ISSUE_NUM} | ${ISSUE_TITLE} | complexity:low, testing:low, epic:cache-engine | PR #${SOURCE_PR} |
```
...
7. **Complexity and testing labels are mandatory** — Every issue must have both a `complexity:` label (low/medium/high) and a `testing:` label (low/medium/high).
8. **Apply milestone and epic when clear** — Use milestones (`V1`, `V2`, `V3`, `Hardening`) and epic labels (`epic:mcp-server`, `epic:cache-engine`, `epic:indexer`, `epic:task-memory`, `epic:telemetry`, `epic:infra`) to organize work.
