...
**Labels each sub-issue will receive:** complexity:low, testing:low, parent:#${PARENT_NUM}
...
```bash
# Always apply both complexity and testing labels (mandatory in this repo)
# Default to complexity:low or complexity:medium; if any sub-issue warrants complexity:high, split further
SUB_LABELS="complexity:${SUB_COMPLEXITY},testing:${SUB_TESTING}"

# Inherit parent's milestone if present
if [ -n "$PARENT_MILESTONE" ]; then
  SUB_LABELS="$SUB_LABELS,milestone:${PARENT_MILESTONE}"
fi

# Inherit any epic:* label from parent
if [ -n "$PARENT_EPIC" ]; then
  SUB_LABELS="$SUB_LABELS,$PARENT_EPIC"
fi
...
| #${SUB_3} | … | complexity:medium, testing:high |
...
8. **Mandatory labels in repo-memory.** Every sub-issue MUST receive both a `complexity:` label (`low`, `medium`, or `high`) and a `testing:` label (`low`, `medium`, or `high`). If any sub-issue warrants `complexity:high`, split it further instead.
9. **Inherit milestone and epic.** Every sub-issue inherits the parent's milestone (V1, V2, V3, Hardening) and any `epic:*` label (e.g., `epic:mcp-server`, `epic:cache-engine`, `epic:indexer`, `epic:task-memory`, `epic:telemetry`, `epic:infra`).
10. **Module-aligned seams.** When proposing sub-issues, align splits with module boundaries (cache engine vs. indexer vs. MCP protocol surface) where possible.
11. **NO attribution.** Follow the project's attribution policy. No "Generated with Claude" / `Co-Authored-By` lines in issue bodies or comments.
