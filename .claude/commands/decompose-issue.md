...
**Labels each sub-issue will receive:** complexity:low, testing:low, parent:${PARENT_MILESTONE}, parent:${PARENT_EPIC}
...
# Inherit parent's epic label if present
...
8. **Mandatory labels in repo-memory.** Every sub-issue MUST receive both a `complexity:` label (`low` or `medium` — if `high` is needed, split further) and a `testing:` label (`low`, `medium`, or `high`). Inherit parent's milestone and epic labels.
9. **Module-aligned seams.** When proposing sub-issue boundaries, align with module boundaries: cache engine, indexer, MCP protocol surface, task memory, persistence layer, etc.
