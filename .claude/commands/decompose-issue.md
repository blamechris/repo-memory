...
/decompose-issue #42 --label epic:cache-engine
```
...
**Labels each sub-issue will receive:** enhancement, complexity:${SUB_COMPLEXITY}
...
Sub-issue titles use the repo commit convention `type(scope): description`. Types: feat, fix, refactor, test, docs, chore. Scopes: server, cache, indexer, memory, graph, telemetry, infra.
...
```bash
# Repo uses "enhancement" + a complexity:low|medium|high label.
SUB_LABELS="enhancement,complexity:${SUB_COMPLEXITY}"

# No parent-link label scheme in this repo — rely on the body "Part of #N" line.
...
8. **NO attribution.** Follow the project's zero-attribution policy. Never add AI/Claude authorship lines or co-author trailers to issue bodies or comments. The user is the sole author.
...
<!-- skill-templates: decompose-issue b4268c2 2026-06-08 -->
